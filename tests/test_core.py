...
def test_not_passing_code_to_inject_raises_error():
    def func():
        pass
...
def test_illegal_value_to_indent_inner_raises_typeerror():
    def func():
        pass
...
def test_injecting_single_line():
    ...
    monki.patch(func, insert={2: 'outlist.append("Injected before end")'})
    outlist = []
...
def test_injecting_multiple_lines():
    ...
    monki.patch(func, insert={1: 'outlist.append("Injected before middle")', 2: 'outlist.append("Injected before end")'})
    outlist = []
    func(outlist)
...
def test_injecting_lines_in_multi_indent_func():
    ...
    monki.patch(func, insert={0: "o.append('Beginning func')",
...
def test_injecting_lines_and_also_wrapping():
    def func(o):
        o.append('Before loop')                  # 0
        for i in range(2):                       # 1
            o.append('In loop')                  # 2
...
class TestIndentingLines():
    ...
    def test_with_indents_as_list(self):
        f = self._make_func()
        monki.patch(f, insert={2: "for i in range(3):"}, indent_lines=[2])
        result = f()
        assert result == ['Starting', 'Iteration: 0', 'Iteration: 1', 'Iteration: 2', 'Ending']

    def test_with_indents_as_dict_level_1(self):
        f = self._make_func()
        monki.patch(f, insert={2: "for i in range(3):"}, indent_lines={2: 1})
        result = f()
        assert result == ['Starting', 'Iteration: 0', 'Iteration: 1', 'Iteration: 2', 'Ending']

    def test_with_indents_as_dict_level_3(self):
        f = self._make_func()
        monki.patch(f, insert={2: "for i in range(3):"}, indent_lines={2: 3})
        result = f()
        assert result == ['Starting', 'Iteration: 0', 'Iteration: 1', 'Iteration: 2', 'Ending']
    ...
    def test_input_validation(self):
        ...
        f = self._make_func()
        with pytest.raises(TypeError) as exc:
            monki.patch(f, insert={2: "for i in range(3):"}, indent_lines=2)  # int is illegal input
        assert error_message in str(exc)
